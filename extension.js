...
function getName(code) {
    for (let v in stockCodes) {
        if (stockCodes[v]['code'] == code) {
            return stockCodes[v]['name'];
        }
    }
    return '';
}
...
function xueqiuFetch() {
    ...
    axios.get(`${baseUrl}${stockList.join(',')}`)
        .then(
            (rep) => {
                try {
                    ...
                    if (result.data) {
                        result.data.map(v => {
                            let item = {};
                            item.code = v['symbol']
                            item.name = getName(item.code);
                            item.price = v['current'];
                            item.percent = v['percent'] / 100.0;
                            item.symbol = v['symbol'];
                            item.high = v['high'];
                            item.low = v['low'];
                            item.open = v['open'];
                            item.yestclose = v['last_close'];
                            item.updown = keepDecimal(item.price - item.yestclose, 2);
                            data.push(item);
                        });
                    }
                    displayData(data);
